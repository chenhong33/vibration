...
QRectF Callout::boundingRect() const //QRectF使用浮点定义矩形 //t拖动必须的
{
    QPointF anchor = mapFromParent(m_chart->mapToPosition(
                                       m_anchor));  //返回图表上与序列指定的序列中的值相对应的位置
    QRectF rect;
...
void Callout::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                    QWidget *widget) //它以局部坐标绘制项的内容
{
    //点选择必须
    Q_UNUSED(option)
    Q_UNUSED(widget)
    QPainterPath
    path; //类提供了一个用于绘制操作的容器，允许构造和重用图形形状
    path.addRoundedRect(m_rect, 5, 5); //将具有圆角的给定矩形矩形添加到路径
    //QPointF anchor = mapFromParent(m_chart->mapToPosition(m_anchor)); //mapToPosition返回图表上与序列指定的序列中的值相对应的位置
    /*if (!m_rect.contains(anchor)) {                             //mapFromParent将此项的父坐标系中的点映射到此项的坐标系，并返回映射的坐标
...
void Callout::mousePressEvent(QGraphicsSceneMouseEvent *event) //鼠标按下
{
    event->setAccepted(true);
}

void Callout::mouseMoveEvent(QGraphicsSceneMouseEvent *event) //鼠标移动
{
