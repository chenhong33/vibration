...
void View::resizeEvent(QResizeEvent *event) //调整大小
{
    /*if (scene()) {  //返回指向视图中当前可视化的场景的指针。如果当前没有可视化的场景，则返回0
        scene()->setSceneRect(QRect(QPoint(0, 0), event->size()));
         ...
         for (Callout *callout : callouts)
             callout->updateGeometry();//尺寸变化
    }*/
    //QGraphicsView::resizeEvent(event);
}

void View::mouseMoveEvent(QMouseEvent *event) //鼠标移动
{
